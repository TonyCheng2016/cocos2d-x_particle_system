...
void CParticle::setBehavior(int iType)
{
	...
	case Infinity:
		...
		t = 2.0f * M_PI * (rand() % 1000) / 1000.0f;
		r = sqrt(9 * cosf(2 * t));
		_Direction.x = r*cosf(t);
		_Direction.y = r*sinf(t);
		_fLifeTime = 1.5f + LIFE_NOISE(0.15f);
		_fIntensity = 1;
		_fOpacity = 255;
		_fSpin = 0;
	...
	case Fun:
		_fVelocity = 15.0f + rand() % 10 / 10.0f;
		t = 2.0f * M_PI * (rand() % 1000) / 1000.0f;
		//r = sinf(t)+ sinf(5 * (t / 2))*sinf(5 * (t / 2))*sinf(5 * (t / 2));
		r = cosf(2*t)+sinf(2*t);
		_Direction.x = r*cosf(t);
		_Direction.y = r*sinf(t);
		_fLifeTime = 1.5f + LIFE_NOISE(0.15f);
...
{
	inlayer.removeChild(_Particle);//清除剩餘的分子
...
void CParticle::setWind(const cocos2d::Point &fWind) {
	_Pos.x += fWind.x;
};
